...
<html lang="en">
    <head>
        ...
        /* iPhone 15 / 15 Pro Max friendly sizing */
        @media (max-width: 430px) {{
            ...
            .sub-note {{
                font-size: clamp(0.95em, 3.8vw, 1.15em);
            }}
            .btn {{
                padding: 12px 28px;
        ...
        @media (max-width: 393px) {{
            ...
            #main-text {{
                font-size: clamp(1.1em, 5.2vw, 1.6em);
            }}
            .btn {{
                padding: 10px 24px;
...
<div id="yt-audio" style="position:fixed;width:1px;height:1px;opacity:0;pointer-events:none;"></div>
<div id="audio-consent" style="position:fixed;top:0;left:0;width:100%;height:100%;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,0.5);z-index:1000;">
    <button id="audio-consent-btn" class="btn">Tap to play music</button>
</div>
<script src="https://www.youtube.com/iframe_api"></script>
        ...
        <script>
            ...
            const emailWebhookYes = 'https://hooks.zapier.com/hooks/catch/26067791/uglht7m/';
            const emailWebhookNo = 'https://hooks.zapier.com/hooks/catch/26067791/uglm37h/';
            const emailRecipient = 'yasmine153@icloud.com';
                    ...
                    events: {{
                        ...
                        'onStateChange': function(ev) {{
                            if (typeof YT !== 'undefined' && ev.data === YT.PlayerState.PLAYING) {{
                                ytPlaying = true;
                                const c = document.getElementById('audio-consent');
                                ...
                                try {{
                                    if (!isiOS) {{
                                        ...
                                        const step = function() {{
                                            ...
                                            if (v < 60) requestAnimationFrame(step);
                                        }};
            ...
            function requestAudioPlay() {{
                try {{
                    if (ytPlayer) {{
                        ...
                        ytPlayer.setVolume && ytPlayer.setVolume(60);
                    }}
                }} catch (e) {{}}
            }}
        ...
        {{
            ...
            function sendEmailNotification(decision) {{
                const endpoint = decision === 'yes' ? emailWebhookYes : (decision === 'no' ? emailWebhookNo : '');
                if (!endpoint) return;
                const payload = {{
                    decision,
                    timestamp: new Date().toISOString(),
                ...
                function init() {{
                    ...
                    document.addEventListener('click', requestAudioPlay, {{ once: true }});
                    document.addEventListener('touchend', requestAudioPlay, {{ once: true }});
                    renderPage();
                    setTimeout(function() {{
                        if (isiOS && !ytPlaying) {{
            ...
            renderPage();
                ...
                function acceptProposal() {{
                    ...
                    textLayer.innerHTML = `
                        ...
                        <p class="sub-note" style="font-size: 0.9em;">And yes, I know what you clicked xx</p>
                    `;
